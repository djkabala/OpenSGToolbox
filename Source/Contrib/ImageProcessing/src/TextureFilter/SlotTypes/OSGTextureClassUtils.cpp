...
//OpenGL depth texture enumerants
...
UInt32 determinTextureFormatClasses(TextureObjChunk* const tex)
{
...
UInt32 determinTextureDataTypeClasses(TextureObjChunk* const tex)
{
    ...
    if(tex != NULL && tex->getImage() != NULL)
    {
        switch(tex->getImage()->getDataType())
        {
            ...
            default:
                SWARNING << "determinTextureDataTypeClasses(): Image Data Type: " 
                                                               << tex->getImage()->getDataType() << " not supported." 
                                                               << std::endl;
                break;
        }
    }
